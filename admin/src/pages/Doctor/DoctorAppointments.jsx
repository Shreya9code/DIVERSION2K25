...
const DoctorAppointments = () => {
  ...
  useEffect(() => {
    if (dToken) {
      ...
      console.log("No doctor token found, authentication issue.");
    }
  }, [dToken]); // Include dToken in dependency array
        ...
        appointments.slice().reverse().map((item, index) => (
          ...
          >
            ...
            <div className="flex items-center gap-3 justify-center">
              ...
              />
              <p className="text-gray-700 font-medium">{item?.userData?.name || "Unknown"}</p>
            </div>

            <p
              className={`font-semibold ${item.payment ? "text-green-600" : "text-red-600"}`}
            >
              {item.payment ? "Online" : "Cash"}
            </p>

            <p className="text-gray-700">{calculateAge(item?.userData?.dob || "2000-01-01")}</p>
