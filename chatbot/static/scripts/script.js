...
function sendUserMessage() {
    ...
    .then(data => {
        ...
        // Check if the response contains goodbye message
        if (isGoodbyeMessage(data.response)) {
            handleGoodbye();
        }
    })
...
function isGoodbyeMessage(response) {
    // Ensure response is a string
    response = String(response);

    // Check if the response contains any goodbye message indicators
    const goodbyeMessages = ['bye', 'cya', 'talk with you later'];
    const lowerResponse = response.toLowerCase(); // Ensure response is lowercased

    for (const message of goodbyeMessages) {
        if (lowerResponse.includes(message)) {
            return true;
        }
    }
    return false;
...
function handleGoodbye() {
    ...
    // Clear input box and replace with new chat button
    document.getElementById('userInput').style.display = 'none'; // Hide input box
    chat.appendChild(newChatButton);
}
...
function displayMessage(message, sender) {
    ...
    icon.src = sender === 'user' ? '/static/images/user-icon.png' : '/static/images/bot-icon.png';
    messageElement.appendChild(icon);

    const textElement = document.createElement('span');
    textElement.classList.add('typing-effect'); // Add a class for typing effect
    ...
    typeEffect(textElement, message);
}
...
function showNewChatButton() {
    ...
    userInputBox.innerHTML = '';  // Clear input box

    const newChatButton = document.createElement('button');
    newChatButton.textContent = '+ New Chat';
    newChatButton.addEventListener('click', () => {
        location.reload();  // Refresh the page to start a new chat
    });

    userInputBox.appendChild(newChatButton);
}
...
function typeEffect(element, text) {
    ...
    function typing() {
        if (index < text.length) {
            ...
            index++;
            setTimeout(typing, 50); // Adjust the typing speed here (milliseconds)
        }
    }
    typing();
}
