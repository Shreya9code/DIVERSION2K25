...
const Login = () => {
  ...
  //const { setToken } = useContext(AppContext); // Access setToken from context
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      const userPayload = { email, password, ...(state === "Sign Up" && { name }) };
      const endpoint = state === "Sign Up" ? "/api/user/register" : "/api/user/login";

      const { data } = await axios.post(backendUrl + endpoint, userPayload);

      if (data.success) {
        console.log(data);
        localStorage.setItem("token", data.token); // ✅ Fix incorrect key
        setToken(data.token);
